...
def processQueue(comq):
        ...
        # each queue item is a tuple which contains  ( adm No, ip address, isDisconnected, cheat_stat)
...
##        while True:
        ...
        cursor.execute("SELECT C_SS_KL FROM T_STUDENT_INFO WHERE C_IP=%s AND C_PORT=%s", (address[0], address[1])) #get with IP and port
        db.commit()
...
##                print "nothing"
        ...
        #reads db to see if there is a termination demand.
        cursor.execute("SELECT C_DISCONNECTED FROM T_STUDENT_INFO WHERE C_IP=%s AND C_PORT=%s", (address[0], address[1])) #get with IP and port
        db.commit()
        demand = [int(record[0]) for record in cursor.fetchall()]
        #print demand
        if demand == [1]: #demand termination
...
##                                print buf
                                ...
                                demand_resp, term_resp = ServerCmd(address)
                                # ServerCmd(demand)
                        ...
                        if str(inst) != "timed out": # timed out exception is okay, else must quit.
                                print " (" + address[0] + ") is DISCONNECTED or HUNG"
                                mesg=(buf,address,False,True)
                                q.put(mesg)
                                break
